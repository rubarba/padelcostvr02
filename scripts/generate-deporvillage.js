/**
 * PadelCost - Gerador Deporvillage (AWIN)
 * ---------------------------------------
 * Lê o feed CSV da Deporvillage na AWIN, filtra produtos relevantes para o
...
const CONFIG = {
  feedUrl: process.env.DEPORVILLAGE_FEED_URL || '',
  feedFile: process.env.DEPORVILLAGE_FEED_FILE || '',
  feedId: process.env.DEPORVILLAGE_FEED_ID || '103842',
  apiKey: process.env.AWIN_API_KEY || '',
  maxProducts: parseInt(process.env.MAX_PRODUCTS_DEPORVILLAGE || process.env.MAX_PRODUCTS || '0', 10),
...
function buildFeedUrl() {
  ...
  if (!CONFIG.apiKey) {
    throw new Error('Define DEPORVILLAGE_FEED_URL ou AWIN_API_KEY no ficheiro .env.');
  }
  ...
  return `https://productdata.awin.com/datafeed/download/apikey/${CONFIG.apiKey}/language/pt/fid/${CONFIG.feedId}/rid/0/hasEnhancedFeeds/0/columns/${columns}/format/csv/delimiter/%2C/compression/gzip/adultcontent/1/`;
}

function fetchFeed(url, redirects = 0) {
  return new Promise((resolve, reject) => {
    console.log('⬇️  A descarregar feed da Deporvillage...');
    https.get(url, (res) => {
...
function mapCategory(row) {
  ...
  const isPadelContext = combined.includes('padel');
  ...
  if (
    ...
    (
      isPadelContext &&
      (
        hasAnyWord(name, ['protector', 'protetor', 'grip']) ||
  ...
  if (name.includes('padel') && hasAnyWord(name, ['zapatilla', 'zapatillas', 'sapatilha', 'sapatilhas', 'calzado', 'shoe', 'shoes', 'tenis'])) return 'sapatilhas';

  if (isPadelContext && (racketNamePattern.test(name) || /^padel\s+/.test(name))) return 'raquetes';

  if (hasAnyWord(name, ['funda'])) return 'acessorios';
...
function toOffer(row) {
  ...
  const offer = {
    id: `deporvillage-${slugify(name)}-${row.aw_product_id || row.merchant_product_id || slugify(row.ean || '')}`,
    name,
    brand: normalizeSpaces(row.brand_name || row.merchant_name || 'Deporvillage'),
    category,
    price,
...
async function main() {
  console.log('🎾  PadelCost - Deporvillage\n');
  ...
  const outputPath = path.resolve(__dirname, CONFIG.outputDir, 'deporvillage-data.js');
  fs.mkdirSync(path.dirname(outputPath), { recursive: true });

  const content = [
    `// PadelCost - Deporvillage`,
    `// Gerado em: ${new Date().toISOString()}`,
    `window.PADELCOST_DEPORVILLAGE_PRODUCTS = ${JSON.stringify(offers, null, 2)};`,
  ].join('\n');
