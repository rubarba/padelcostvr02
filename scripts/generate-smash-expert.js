/**
 * PadelCost - Gerador Smash Expert (AWIN)
 * ---------------------------------------
 * Lê o feed CSV da Smash Expert na AWIN, filtra produtos relevantes para o
...
const CONFIG = {
  feedUrl: process.env.SMASH_EXPERT_FEED_URL || '',
  feedFile: process.env.SMASH_EXPERT_FEED_FILE || '',
  feedId: process.env.SMASH_EXPERT_FEED_ID || '93547',
  apiKey: process.env.AWIN_API_KEY || '',
  maxProducts: parseInt(process.env.MAX_PRODUCTS_SMASH_EXPERT || process.env.MAX_PRODUCTS || '0', 10),
...
function buildFeedUrl() {
  ...
  if (!CONFIG.apiKey) {
    throw new Error('Define SMASH_EXPERT_FEED_URL ou AWIN_API_KEY no ficheiro .env.');
  }
...
function fetchFeed(url, redirects = 0) {
  return new Promise((resolve, reject) => {
    console.log('⬇️  A descarregar feed da Smash Expert...');
    https.get(url, (res) => {
...
function mapCategory(row) {
  ...
  if (
    sourceCategory.includes('grip de padel') ||
    sourceCategory.includes('surgrip de padel') ||
    sourceCategory.includes('protection raquette') ||
    sourceCategory.includes('bande de protection') ||
    sourceCategory.includes('cordon') ||
    sourceCategory.includes('dragonnes') ||
    sourceCategory.includes('accessoire') ||
    sourceCategory.includes('balle de padel') ||
    sourceCategory.includes('tube balles de padel') ||
    sourceCategory.includes('pressurisateur') ||
    sourceCategory.includes('préssurisateur') ||
    sourceCategory.includes('ramasse balle') ||
    sourceCategory.includes('ramasse-balle') ||
    sourceCategory.includes('panier de balles')
  ) return 'acessorios';
...
function toOffer(row) {
  ...
  const offer = {
    id: `smash-expert-${slugify(name)}-${row.aw_product_id || row.merchant_product_id || slugify(row.ean || '')}`,
    name,
    brand: normalizeSpaces(row.brand_name || row.merchant_name || 'Smash Expert'),
    category,
    price,
...
async function main() {
  console.log('🎾  PadelCost - Smash Expert\n');
  ...
  const outputPath = path.resolve(__dirname, CONFIG.outputDir, 'smash-expert-data.js');
  fs.mkdirSync(path.dirname(outputPath), { recursive: true });

  const content = [
    `// PadelCost - Smash Expert`,
    `// Gerado em: ${new Date().toISOString()}`,
    `window.PADELCOST_SMASH_EXPERT_PRODUCTS = ${JSON.stringify(offers, null, 2)};`,
  ].join('\n');
